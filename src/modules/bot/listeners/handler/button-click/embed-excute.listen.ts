...
@Injectable()
export class EmbedMessageExcuteListener {
  private readonly logger = new Logger(EmbedMessageExcuteListener.name);
  ...
  @OnEvent(BOT_BUTTON_ACTION_FEATURES.EMBED_EXECUTE)
  async handleMessageButtonClick(payload: {
    payload: ButtonClickPayload;
    params: string[];
  }) {
    this.logger.debug('handleMessageButtonClick');
    const {
    ...
    this.mezonService
      .updateChatMessage({
        ...
        content: generateChannelMessageContent({
          message: (user?.displayName ?? user_id) + ' updated message' + `\n Extra data: ${JSON.stringify(JSON.parse(payload.payload.extra_data), null, 2)}`,
          blockMessage: true,
        }),
      ...
      .then(res => {
        this.logger.debug('SUCCESS:' + JSON.stringify(res, null, 2));
      })
      .catch(res => {
